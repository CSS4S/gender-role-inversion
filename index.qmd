...
- Set genders randomly. Note this means agents won't have the same number of same- and opposite-gender neighbors as one another.
- Set the deviance penalty $\delta$ and an initial fraction of men as followers to see what initial support for bucking the norm can overcome what penalty.
...
- Our primary explanatory variable is the initial prevalence of gender role *inverters*, call it $I\in[0, 1]$ for which we will test values from 0.1 to 0.9 in steps of 0.2 to start. We observe the prevalence of women leaders over time 
